...
theorem preservedUnderCoarsening_iff_neg_preservedUnderRefinement
  ...
  constructor
  · intro hcoarse f g hfg hnZg hZf
    exact hnZg (hcoarse f g hfg hZf)
  · intro hrefine f g hfg hZf
    by_contra hnZg
    exact hrefine f g hfg hnZg hZf

/-! ## Implication Between Axioms -/

/-- An axiom `Z₁` implies axiom `Z₂` if every rule satisfying `Z₁` also satisfies `Z₂`. -/
